...
import com.ghy.common.util.reflection.ReflectionUtils;
...
public class SimpleHibernateDao<T, PK extends Serializable> {
	...
	/**
	 * 可以根据传过来的class类进行绑定 不一定非是T
	 * @param entity
	 * @param id
	 * @return
	 */
	public <X>X get(final Class<X> entity,final PK id){
		try {
			Assert.notNull(id, "id不能为空");
		} catch (AssertionError e) {
			return null;
		}
		Object o = getSession().get(entity, id);
		return o == null ? null : (X) o;
	}

	public <X>  void  release(X entity){
		 this.getSession().evict(entity);
	}
	
	public <X> Serializable saveX(X entity){
	...
	/**
	 * 根据查询SQL与参数列表创建SQLQuery对象. 与find()函数可进行更加灵活的操作.
	 * 
	 * @param values
	 *            命名参数,按名称绑定.
	 */
	 ...
	 */
	public SQLQuery createSqlQuery(final String queryString,
			final Map<String, ?> values,final Map<String, ?> values2) {
		Assert.hasText(queryString, "queryString不能为空");
		SQLQuery query = getSession().createSQLQuery(queryString);
		 if ((values != null) && (values.size() > 0)) {
			query.setProperties(values);
	 ...
	 /**
		 ...
		 * @return
		 */
		public List<Object[]> findBySql(String sql, Map<String, ?> params,Map<String, ?> params2) {
			return createSqlQuery(sql,params,params2).list();
		}
}